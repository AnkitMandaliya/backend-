...
export const registerUser = asyncHandler(async (req, res) => {
    ...
    const avatarImage = req.file?.avatar[0].path;
    const coverImage = req.file?.coverImage[0].path;
    if(!avatarImage || !coverImage){
        throw new ApiError(400,"Avatar and Cover image are required");
    }
    //upload image to cloudinary
    const avatar = await uploadToCloudinary(avatarImage,"Avatar");
    const coverImg = await uploadToCloudinary(coverImage,"CoverImage");
